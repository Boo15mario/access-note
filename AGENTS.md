...
<architecture>
- Project boundaries:
  - `src/AccessNote`: host app (MainWindow partials + composition root wiring only).
  - `src/AccessNote.Core`: domain models, storage, and shared contracts.
  - `src/AccessNote.Shell`: shell routing/input/menu/startup/exit orchestration and shell dialogs.
  - `src/AccessNote.Applets.*`: applet-specific behavior and dialogs (e.g., Notes, Settings).
...
- Settings screen state is split by concern:
  ...
  - `SettingsOptionListBuilder` (option construction + row text)
  - `SettingsSelectionSynchronizer` (selection suppression/sync)
  - `SettingsOptionAnnouncer` (spoken announcements)
- For new applets, prefer scaffold flow: `scripts/new_applet.sh <AppletName>` + `templates/applet/`.
</architecture>
</repo_notes>
